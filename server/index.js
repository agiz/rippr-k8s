...
apiRoutes.post('/searchTest', async (req, res) => {
  ...
  const language = 'language' in req.body || 'language' in req.body && req.body.language.length > 0 ? `p1.language IN (${req.body.language.map(x => `${sqlstring.escape(x)}`).join(',')})` : 'true'
